...
class AuthBackend(object):
    ...
    def parse_auth_token_from_request(self, auth_header, headers={}):
        """
...
class JWTAuthBackend(AuthBackend):
    ...
    def _decode_jwt_token(self, req):
        ...
        token = self.parse_auth_token_from_request(auth_header=auth_header, headers=req.headers)
...
class BasicAuthBackend(AuthBackend):
    ...
    def _extract_credentials(self, req):
        ...
        token = self.parse_auth_token_from_request(auth_header=auth, headers=req.headers)
        try:
...
class TokenAuthBackend(BasicAuthBackend):
    ...
    def _extract_credentials(self, req):
        auth = req.get_header('Authorization')
        return self.parse_auth_token_from_request(auth_header=auth, headers=req.headers)
...
class AuthBackend_async(object):
    """
    Base Class for all authentication backends. If successfully authenticated must
    return the authenticated `user` object. In case authorization header is
    not set properly or there is a credential mismatch, results in an
    ...
    async def parse_auth_token_from_request(self, auth_header, headers={}):
        """
        Parses and returns Auth token from the request header. Raises
        `falcon.HTTPUnauthoried exception` with proper error message
        """
    ...
    async def get_auth_token(self, user_payload):
        """
        Returns a authentication token created using the provided user details
    ...
    async def get_auth_header(self, user_payload):
        """
        Returns the value for authorization header
        Args:
            user_payload(dict, required): A `dict` containing required information
...
class JWTAuthBackend_async(AuthBackend_async):
    """
    Token based authentication using the `JSON Web Token standard <https://jwt.io/introduction/>`__
    Clients should authenticate by passing the token key in the `Authorization`
    HTTP header, prepended with the string specified in the setting
    ...
    async def _decode_jwt_token(self, req):

        # Decodes the jwt token into a payload
        auth_header = req.get_header('Authorization')
        token = await self.parse_auth_token_from_request(auth_header=auth_header, headers=req.headers)
    ...
    async def authenticate(self, req, resp, resource):
        ...
        object if successful else raise an `falcon.HTTPUnauthorized exception`
        """
        payload = await self._decode_jwt_token(req)
...
class BasicAuthBackend_async(AuthBackend_async):
    """
    Implements `HTTP Basic Authentication <http://tools.ietf.org/html/rfc2617>`__
    Clients should authenticate by passing the `base64` encoded credentials
    `username:password` in the `Authorization` HTTP header, prepended with the
    ...
    async def _extract_credentials(self, req):
        auth = req.get_header('Authorization')
        token = await self.parse_auth_token_from_request(auth_header=auth, headers=req.headers)
        try:
            token = base64.b64decode(token).decode('utf-8')
    ...
    async def authenticate(self, req, resp, resource):
        ...
        object if successful else raise an `falcon.HTTPUnauthorized exception`
        """
        username, password = await self._extract_credentials(req)
        user = await self.user_loader(username, password)
        if not user:
            raise falcon.HTTPUnauthorized(
                description='Invalid Username/Password',
                headers=req.headers)
...
class TokenAuthBackend_async(BasicAuthBackend_async):
    """
       Implements Simple Token Based Authentication. Clients should authenticate by passing the token key in the
           "Authorization" HTTP header, prepended with the string "Token ".  For example:
    ...
    def __init__(self, user_loader,
        ...
        super(TokenAuthBackend_async, self).__init__(user_loader, auth_header_prefix)

    async def _extract_credentials(self, req):
        auth = req.get_header('Authorization')
        return await self.parse_auth_token_from_request(auth_header=auth, headers=req.headers)

    async def authenticate(self, req, resp, resource):
        token = await self._extract_credentials(req)
        user = await self.user_loader(token)
        if not user:
            raise falcon.HTTPUnauthorized(
                description='Invalid Token',
                headers=req.headers)
...
class NoneAuthBackend_async(AuthBackend):
    """
    Dummy authentication backend.
...
class MultiAuthBackend_async(AuthBackend_async):
    """
    A backend which takes two or more ``AuthBackend`` as inputs and successfully
    authenticates if either of them succeeds else raises `falcon.HTTPUnauthorized exception`
    ...
    def __init__(self, *backends):
        ...
        for backend in backends:
            if not isinstance(backend, AuthBackend) and not isinstance(backend, AuthBackend_async):
                raise ValueError(('Invalid authentication backend {0}.'
                                 'Must inherit `falcon.auth.backends.AuthBackend` or `falcon.auth.backends.AuthBackend_async`')
                                 .format(backend))
        ...
        for backend in self.backends:
            try:
                if backend.__class__.__name__.endswith("_async"):
                    user = await backend.authenticate(req, resp, resource)
                else:
                    user = backend.authenticate(req, resp, resource)
                if user:
                    return user
    ...
    async def get_auth_token(self, user_payload):
        for backend in self.backends:
            try:
                if backend.__class__.__name__.endswith("_async"):
                    return await backend.get_auth_token(user_payload)
                else:
                    return backend.get_auth_token(user_payload)
            except Exception:
                pass
